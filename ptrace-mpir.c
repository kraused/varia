...
/*
static unsigned long long proc_get_base_from_auxv(int pid)
{
	FILE *f;
	char fn[1024];
	Elf64_auxv_t buf;

	snprintf(fn, sizeof(fn), "/proc/%d/auxv", pid);
	f = fopen(fn, "r");

	do {
		fread(&buf, sizeof(Elf64_auxv_t), 1, f);
	} while ((AT_BASE != buf.a_type) && (AT_NULL != buf.a_type));

	fclose(f);

	if (AT_BASE == buf.a_type) {
		return buf.a_un.a_val;
	}
